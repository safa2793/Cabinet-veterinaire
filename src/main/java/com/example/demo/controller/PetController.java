...
public class PetController {
	...
	public String PetsShowPanel(@PathVariable int customerid, Map<String, Object> map) throws SQLException {
		...
		if(customer.isPresent()) {
			...
			map.put("title", "Animaux appartenant au client");
			...
			map.put("title", "Client");
			map.put("customers", customers);
			map.put("message", " Aucun Enregistrement Trouvé.");
	...
	public String PetsShowPanel2(@RequestParam("name") String name,@PathVariable int customerid, Map<String, Object> map) throws SQLException {
		...
		if(customer.isPresent()) {
			...
			if(pets.size()>0) {
				map.put("pets", pets);
				map.put("message", "Des enregistrements ont été trouvés");
			}
			else {
				map.put("message", name+" aucun animal par le nom trouvé.");
			...
			List<Customer> customers = customerServiceImp.findAll();
			map.put("title", "Clients");
			map.put("customers", customers);
			map.put("message",  "Il n'y a pas d'enregistrement avec ce nom. id : " + customerid);
			return "customer/customers";
		}
	...
	public String InsertPet(@PathVariable int customerid, Map<String, Object> map,
		...
		if (!pet.getName().equals("") || !pet.getProblem().equals("") ) {
			...
			if(customer.isPresent()) {
				if (result.hasErrors()) {
					map.put("message", " Une erreur est survenue.");				

				} else {
					pet.setCustomer(customer.get());
					petRepository.save(pet);
					map.put("message", "inscription réussie");
				}
				List<Pet> pets = petServiceImp.findByCustomer(customer.get());
				map.put("customer", customer.get());
				map.put("pet", new Pet());
			...
			if(customer.isPresent()) {
				...
				map.put("customer", customer.get());
				map.put("pet", new Pet());
				map.put("message", "Remplissez les champs vides");
				map.put("pets", pets);
	...
	public String DeletePet(@PathVariable long pet_id, Map<String, Object> map, @Valid @ModelAttribute("pet") Pet pet,
		...
		if(selected_pet.isPresent()) {
			...
			if(control==true) {
				map.put("message","L'enregistrement a été supprimé avec succès.");
			}else {
				map.put("message","Une erreur est survenue.");
			}
			...
			map.put("title", "Clients");
			map.put("customers", customers);
			map.put("message","Registre de l'animal introuvable.");
			return "customer/customers";
		}
